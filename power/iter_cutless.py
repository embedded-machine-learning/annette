...
for x in range(num):
    print(x)
    #IMX8
    net = f'mobilenetv2-7-sim_{str(x)}'
    power_file= net+'.npy'
    power_path = Path('data','imx8_mobilenetv2-7-sim')
    tresh = 3.5
    extend = 0.2
    ...
    net = f'data_{str(x)}'
    #net = f'deeplabv3_mobilenet_v3_large-sim_{str(x)}_dec'
    net = f'mobilenet_tf2_{str(x)}'
    net = f'mobilenetv2-7-sim_{str(x)}'
    power_file= net+'.npy'
    power_path = Path('data','edge_tpu_mobilenet_tf2')
    power_path = Path('data','edgetpu_std_mobilenetv2-7-sim')
    tresh = 1.05
    #power_path = Path('data','imx8_deeplabv3_mobilenet_v3_large-sim')
    extend = 0.02
    smoothed = 11
    ...
    result[x] = processing.extract_power_profile(power_file, power_path, duration, sample_rate = rate, start_thresh=0.1, end_thresh=0.1, peak_thresh=0.7, vis = v, multiplier=1.0, redefine_start=False, smooth=smoothed)
...
for i in range(0,num):
    ...
    r1 = result[i1]
    r2 = result[i2][:-int(extend*rate)]
    dist, r1, r2 = processing.align(r1, r2, shift, vis=False)

    length = np.min([len(r1),len(r2)])
    #plot correlation
    divergece = np.abs(smooth(r1[:length],ks)-smooth(r2[:length],ks))
    divergece = smooth(divergece)
    #divergece = np.log(divergece)[:-ks]
    divergece = divergece[:-ks]

    if np.max(divergece) > max_th:
        maximum = np.argmax(divergece)
    else:
        maximum = len(divergece)

    # find where divergance below threshold
    div = np.where(divergece[:maximum] < div_th)[0]
