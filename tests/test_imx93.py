...
def test_optimize_network(network="annette_bench1.pb"):
    ...
    imx93_obj.optimize_network(test_net, source_fw = "tf", network = network, input_node = "data", save_folder = get_database('benchmarks','tmp'))
    assert True
...
def test_measure_network(network="cf_reid"):
    imx93_obj = imx93.inference.imx93Class(get_database('configs','imx93.yaml'))
    matcher.measure_network(optimize = imx93.inference.optimize_network, execute = imx93.inference.run_network_new, parse = imx93.parser.r2a, network = network, framework = 'onnx')

def test_measure_annette_network(network="cf_reid"):
    matcher.measure_annette_network(optimize = imx93.inference.optimize_network, execute = imx93.inference.run_network_new, parse = imx93.parser.r2a, network = network)

def test_measure_destruct_annette_network(network="cf_reid"):
    matcher.measure_destruct_annette_network(optimize = imx93.inference.optimize_network, execute = imx93.inference.run_network_new, parse = imx93.parser.r2a, network = network, config="config_v6.csv")
...
def main():
    ...
    test_all(network='annette_bench_avg',config='config_v7_2.csv',start=0, end=10, vis=False)
    test_all(network='annette_bench_conv_avg',config='config_v7_2.csv',start=0, end=10, vis=False)
    test_all(network='annette_bench_avg',config='config_v7_3.csv',start=0, end=10, vis=False)
    test_all(network='annette_bench_conv_avg',config='config_v7_3.csv',start=0, end=10, vis=False)
