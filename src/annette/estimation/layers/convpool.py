...
class ConvPoolLayer(BaseLayer):
    ...
    def compute_parameters(self, layer = None):
        ...
        self.layer['num_outputs'] = reduce(lambda x, y: x*y, self.layer['Pool']['output_shape'][1:])
            ...
            if self.architecture:
                if 'h_par' in self.architecture:
                    ...
                    if 'h_alpha' in self.architecture:
                        self.layer['h_eff'] = 1/(self.architecture['h_alpha'] + 1/self.layer['h_eff'] * (1-self.architecture['h_alpha']))
                ...
                if 'c_par' in self.architecture:
                    if self.architecture['c_par'] < 1:
                        ...
                        if self.layer['kernel_shape'][2] < self.architecture['c_par']:
                            #self.layer['c_eff'] = self.layer['kernel_shape'][2]*np.floor(self.architecture['c_par']/self.layer['kernel_shape'][2])/self.c_par
                            self.layer['c_eff'] = 1
                        else:
                    ...
                    if 'c_alpha' in self.architecture:
                        self.layer['c_eff'] = 1/(self.architecture['c_alpha'] + 1/self.layer['c_eff'] * (1-self.architecture['c_alpha']))
                ...
                if 'f_par' in self.architecture:
                    ...
                    if 'f_alpha' in self.architecture:
                        self.layer['f_eff'] = 1/(self.architecture['f_alpha'] + 1/self.layer['f_eff'] * (1-self.architecture['f_alpha']))
            else:
                print("noarch")
        except:
            print("noarch")
    ...
    def estimate_roofline(self):
        """returns roofline estimated ConvPoolLayer execution time (ms)"""
