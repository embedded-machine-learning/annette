...
class Optimizer():
    ...
    def merge_cond(self, graph, layer):
        ...
        c = graph.model_spec['layers'][layer]['children'][0]
        try:
            primary = self.layer_classes[p_layertype].compute_nums(graph.model_spec['layers'][layer])
        except:
            primary = graph.model_spec['layers'][layer]

        s_layertype = graph.model_spec['layers'][c]['type']
        try:
            secondary = self.layer_classes[s_layertype].compute_nums(graph.model_spec['layers'][c])
        except:
...
class Mapping_model():
    ...
    def add_optimizer(self, name, prim_type, sec_type, out_type, est_model = None, conv_dict = None, fuse_cond = None):
        self.optimizers[name] = Optimizer(name, prim_type, sec_type, out_type, est_model, conv_dict, fuse_cond)
    ...
    def from_json(cls,filename):
        ...
        for l in opt_desc.items():
            print(l[0])
            print(l[1])
            name = l[1]['name']; prim_type = l[1]['prim_type']; sec_type = l[1]['sec_type']; out_type = l[1]['out_type']
            opts = {}
            ...
            output_model.add_optimizer(name, prim_type, sec_type, out_type, est_model = opts['est_model'], conv_dict = opts['conv_dict'], fuse_cond = opts['fuse_cond'])
